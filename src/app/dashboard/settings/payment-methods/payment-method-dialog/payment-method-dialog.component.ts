...
import { payMethodEditRequest, payMethodCreateRequest } from 'src/app/reducers/settings/actions';
  ...
  save() {
    if (this.formGroup.valid) {
      ...
      if (values && values.id !== 0) {
        this.store.dispatch(payMethodEditRequest({ pm: values }));
      } else {
        this.store.dispatch(payMethodCreateRequest({ pm: values }));
      }
    }
  }
}
